...
<p align="center">
  <img src="./images/atomicity.png" alt="Atomicity">
</p>
...
**যত বেশি Isolation -> তত বেশি লকিং -> তত কম Performance। এখানে কোনো "সব দিক ভালো" অপশন নেই।**

- Read Uncommitted

**সুবিধা:** সবচেয়ে দ্রুত, কোনো lock নেই বললেই চলে

**অসুবিধা:** Dirty Read হতে পারে — মানে এমন data পড়তে পারো যা পরে rollback হয়ে গেছে

**কখন ব্যবহার করবে:** প্রায় কখনোই না। শুধু approximate analytics-এ যেখানে একটু ভুল data হলেও সমস্যা নেই

- Read Committed

**সুবিধা:** Dirty Read থেকে নিরাপদ, performance ভালো

**অসুবিধা:** একই transaction-এ দুবার read করলে ভিন্ন result পেতে পারো (Non-Repeatable Read)

**কখন ব্যবহার করবে:** বেশিরভাগ সাধারণ application-এ। PostgreSQL-এর default এটাই

- Repeatable Read

**সুবিধা:** একই row বারবার read করলে একই data পাবে

**অসুবিধা:** নতুন row insert বা delete হলে দেখতে পাবে (Phantom Read)

**কখন ব্যবহার করবে:** Report generation বা এমন জায়গায় যেখানে consistent row দরকার। MySQL-এর default এটাই

- Snapshot

**সুবিধা:** তিনটি anomaly থেকেই নিরাপদ। Reader আর Writer একে অপরকে block করে না (MVCC)

**অসুবিধা:** Memory বেশি লাগে কারণ পুরনো snapshot রাখতে হয়। Write skew anomaly হতে পারে বিশেষ ক্ষেত্রে

**কখন ব্যবহার করবে:** যেখানে read-heavy workload এবং concurrency দরকার। SQL Server-এর default

- Serializable

**সুবিধা:** সবচেয়ে নিরাপদ, কোনো anomaly নেই

**অসুবিধা:** Conflict হলে transaction rollback হয়ে যায়, throughput অনেক কমে যায়

**কখন ব্যবহার করবে:** Financial transaction, bank transfer — যেখানে correctness সবকিছুর আগে

## Durability

ACID মডেলের 'D' মানে হলো Durability। এটি নিশ্চিত করে যে, একবার যদি কোনো ট্রানজেকশন সফলভাবে সম্পন্ন (Commit) হয়, তবে সিস্টেম ক্র্যাশ করলে সেই ডেটা আর হারাবে না।

### Write-Ahead Log (WAL)

এর মাধ্যমে ডাটাবেস Durability নিশ্চিত করে। WAL একটি logging mechanism যেখানে database page disk এ write হওয়ার আগে পরিবর্তনগুলো প্রথমে log file এ লেখা হয়।

আধুনিক ডাটাবেস সিস্টেম যেমন MySQL-এ WRITE (Transaction ব্যবহার করে কিংবা না করে) অপারেশন প্রথমে মেমরির ভিতরে থাকা buffer pool-এ প্রয়োগ করা হয়। এরপর ডাটাবেস সেই পরিবর্তনটি বর্ণনা করে একটি log record(change এর description) তৈরি করে এবং সেটিকে মেমরির মধ্যে থাকা WAL buffer-এ সংরক্ষণ করে।

Transaction commit হওয়ার আগে এই log-টি Write-Ahead Log (WAL) ফাইলে ডিস্কে flush করা হয়।

Transaction commit হওয়ার পরে পরিবর্তিত data page গুলো buffer pool-এ থাকে এবং এগুলো পরে background process দ্বারা মূল database table-এ insert করে দেয়।

যেহেতু WAL sequential ভাবে (ক্রম অনুযায়ী) লেখা হয়, তাই এটি random data page update করার তুলনায় অনেক দ্রুত।

যদি কোনো crash ঘটে, তাহলে ডাটাবেস WAL replay করে committed transaction গুলো পুনরুদ্ধার করতে পারে, যার ফলে durability নিশ্চিত হয়।
