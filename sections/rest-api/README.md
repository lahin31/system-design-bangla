...
প্রতিটি Request self-contained হবে। Server কোনো Client Session Memory ধরে রাখবে না। Authentication, Authorization বা প্রয়োজনীয় Context প্রতিটি Request-এর সাথেই পাঠাতে হবে।
...
- **Resource Identification**: প্রতিটি Resource কে একটি Unique URI দ্বারা চিহ্নিত করতে হবে। যেমন, `/users/{id}`, `/products/{id}`।
- **Manipulation through Representations**: Client যখন কোনো Resource এর Representation (যেমন JSON) পায়, তখন সেই Representation এর মাধ্যমেই Client সেই Resource কে modify বা delete করতে পারবে।
- **Self-descriptive Messages**: প্রতিটি Request এবং Response এ যথেষ্ট তথ্য থাকতে হবে, যাতে Client বা Server অন্য কোনো অতিরিক্ত তথ্য ছাড়াই সেই Message কে process করতে পারে। যেমন, HTTP Headers এ Content-Type উল্লেখ করা থাকে, যা বলে দেয় Body টি কোন format এ আছে (JSON/XML)।
...
POST Method মূলত Server-কে কোনো Request process করার জন্য ব্যবহৃত হয়। সবচেয়ে সাধারণ ব্যবহার হলো নতুন Resource তৈরি করা।
...
PUT Method সাধারণত নির্দিষ্ট Resource সম্পূর্ণ replace বা update করার জন্য ব্যবহৃত হয়।
...
POST এবং PUT এর মধ্যে পার্থক্য হল, POST Method মূলত Server-কে কোনো Request process করার জন্য ব্যবহার করা হয়। সবচেয়ে সাধারণ ব্যবহার হলো নতুন Resource তৈরি করা, তবে POST শুধু Resource তৈরির মধ্যেই সীমাবদ্ধ নয়। Server-side processing প্রয়োজন এমন কাজেও POST ব্যবহার করা হয়। PUT হল idempotent মানে রিসোর্স যদি ইতিমধ্যে থাকে তাহলে সে আর নতুন রিসোর্স তৈরি করবে না।
...
- Error Handling
  ...
  - REST API সাধারণত Weakly Typed বা স্কিমা বিহীন হয়। রিকোয়েস্ট-রেসপন্সের টাইপ রানটাইমে ভ্যালিডেশন করতে হয়।
...
Idempotent API বলতে বোঝায় - একটি নির্দিষ্ট API রিকোয়েস্ট একবার কল করলে সার্ভারে যে পরিবর্তন বা স্টেট ট্রান্সফার হয়, একই প্যারামিটার দিয়ে সেই রিকোয়েস্টটি বারবার (১০০ বারও) কল করলেও সার্ভারের স্টেটে ঠিক একই প্রভাব থাকবে, নতুন কোনো পরিবর্তন হবে না। অর্থাৎ, একাধিকবার একই রিকোয়েস্ট পাঠালেও চূড়ান্ত ফলাফল সবসময় একই থাকবে।
...
- REST API এর Principle গুলো কি কি?
...
- API এর Latency 100ms থেকে 2s বেড়ে গেলে আপনি কি করবেন?
- আপনার API তে ট্রাফিক এর পরিমাণ হঠাৎ করে 10x বৃদ্ধি পেলে, আপনি প্রথমে কি চেক করবেন?
- আপনি কিভাবে API কিভাবে ডিসাইন যাতে ভবিষ্যতে নতুন ফিচার যোগ বা পরিবর্তন করলেও পুরনো ক্লায়েন্টগুলো ভেঙে না যায় (break না করে)?
