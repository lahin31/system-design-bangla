...
Database Indexing নিয়ে আমরা গভীরভাবে অধ্যায় ৩ এ আলোচনা করা হয়েছে।
...
## SQL Data Modeling/Schema Design best practices

### Normalize / DeNormalize

Normalization মানে ডেটাকে ছোট ছোট টেবিলে ভাগ করে redundancy কমানো। আর Denormalization মানে performance-এর জন্য ইচ্ছাকৃতভাবে কিছুটা redundancy রাখা।

যদি data integrity বেশি দরকার হয় — Normalize করুন। আর যদি read performance বেশি গুরুত্বপূর্ণ হয় (যেমন reporting বা analytics) — সেখানে Denormalize করা যেতে পারে। দুটোর মধ্যে কোনটা নেবেন সেটা নির্ভর করে আপনার system-এর চাহিদার উপর।

### সঠিক Data Type

না বুঝে data type দিলে storage waste হয়, performance কমে। কোনটা কখন ব্যবহার করবেন -

- TINYINT → ছোট integer (যেমন age, status flag)
- INT → সাধারণ integer কাজে
- UNSIGNED → যখন negative value দরকার নেই, এটা দিলে positive range দ্বিগুণ হয়ে যায়
- BIGINT → অনেক বড় সংখ্যার জন্য (যেমন transaction ID)
- DECIMAL → টাকা-পয়সার হিসাব বা exact decimal দরকার হলে
- VARCHAR(256) → ছোট string (নাম, শিরোনাম)
- TEXT → বড় text (description, comment)
- DATETIME → Date ও Time, কিন্তু timezone ছাড়া
- TIMESTAMP → Date ও Time, timezone সহ
- ENUM → যখন নির্দিষ্ট কিছু string value-র মধ্যেই সীমাবদ্ধ থাকবে (যেমন status: ‘active’, ‘inactive’)

<p align="center">
  <img src="./images/data_type.png" alt="Data Type">
</p>

### Primary Key ব্যবহার করুন

প্রতিটা টেবিলে Primary Key থাকা বাধ্যতামূলক ধরে নিন। এটা ছাড়া row uniquely identify করা যায় না, পরে join বা relation তৈরিতে সমস্যায় পড়বেন।

### UNIQUE Constraint ব্যবহার করুন

যে column-এর সব value আলাদা আলাদা হওয়া দরকার, সেখানে UNIQUE constraint দিন। যেমন email বা username — দুইজন user-এর একই email থাকা উচিত না।

### Index দেওয়ার আগে বিশ্লেষণ করুন

Index দিলেই যে সব সমস্যা সমাধান হয়ে যাবে — এই ধারণা ভুল। Index দেওয়ার আগে Cardinality বুঝতে হবে —

- High Cardinality (যেমন email, phone number — প্রায় সব value আলাদা) → এখানে index ভালো কাজ করে
- Low Cardinality (যেমন gender, status — মাত্র কয়েকটা unique value) → এখানে index তেমন কাজে আসে না, বরং overhead বাড়ে

তাই blindly index না দিয়ে আগে query pattern আর data analyze করুন।

### Foreign Key দিয়ে Referential Integrity রক্ষা করুন
Foreign Key শুধু relation তৈরির জন্য না — এটা নিশ্চিত করে যে একটা টেবিলে এমন কোনো value না থাকে যেটা referenced টেবিলে নেই।

দুটো গুরুত্বপূর্ণ behavior মাথায় রাখুন —

- CASCADE → parent delete হলে child-ও automatically delete হবে
- SET NULL → parent delete হলে child-এর foreign key column NULL হয়ে যাবে

কোনটা ব্যবহার করবেন সেটা আপনার business logic-এর উপর নির্ভর করে।

### Naming Convention ঠিক রাখুন

Table name lowercase আর plural রাখুন — যেমন users, orders, product_categories। আর column name-এ snake_case অথবা camelCase — যেটাই বেছে নিন, পুরো project জুড়ে একটাই pattern follow করুন। মিক্স করলে পরে maintain করা ঝামেলা হয়।

### Junction Table ব্যবহার করুন Many-to-Many Relation-এ

দুটো টেবিলের মধ্যে many-to-many relation থাকলে সরাসরি করা যায় না। এজন্য একটা আলাদা Junction Table বানাতে হয়। যেমন students আর courses — একজন student অনেক course নিতে পারে, আবার একটা course-এ অনেক student থাকতে পারে। এদের মাঝে student_courses নামে junction table বানাবেন।

### Audit Column রাখুন

কে কখন data তৈরি করেছে বা পরিবর্তন করেছে — এটা track করতে এই চারটা column রাখা ভালো অভ্যাস — created_at, updated_at, created_by, updated_by। পরে debugging বা audit trail দরকার হলে এগুলো অনেক কাজে আসবে।

### Soft Delete রাখুন

ডেটা সরাসরি delete না করে Soft Delete রাখা বেশিরভাগ ক্ষেত্রে নিরাপদ। এজন্য দুটো column যোগ করুন —
is_deleted, deleted_at। 

is_deleted = true, মানে record টা deleted হিসেবে mark হয়েছে, কিন্তু database থেকে আসলে মুছে যায়নি। পরে দরকার হলে recover করা যাবে, আর historical data-ও intact থাকবে।
...
এখানে genre কলামটায় ইনডেক্স আছে (InnoDB তে এটা secondary index)।
কিন্তু সমস্যা হলো — কুয়েরিতে যেহেতু \* আছে তার মানে সকল কলাম প্রয়োজন, যেগুলো ওই genre ইনডেক্সের সাথে নেই।

তাহলে কী হয়?

- প্রথমে MySQL genre ইনডেক্স ব্যবহার করে matching row গুলোর Primary Key (PK) খুঁজে বের করে।
- তারপর প্রতিটা row এর জন্য মূল টেবিল (clustered index / heap) এ গিয়ে সকল কলাম পড়ে নিয়ে আসবে।
- মানে অতিরিক্ত disk/page read লাগে → I/O বাড়ে → query ধীর হয়।
...
## id auto-increment ব্যবহার করার পরে সর্বোচ্চ মানে চলে এলে কি হয়?

২০২১ সালের ৫ মে এবং ২০২১ সালের ২৭ নভেম্বর, GitHub দুইটি ছোট আউটেজ(Outage) হয়েছিল, যা উভয়ই ডাটাবেস স্কিমা সীমাবদ্ধতার কারণে ঘটে।
...
প্রথম আউটেজ ID কলাম তার সর্বোচ্চ মানে চলে এসেছিলো, পরবর্তী সময়ে যখন নতুন row ইন্সার্ট করার চেষ্টা হয়েছিল তখন আর সেই row ইন্সার্ট সম্ভব হয় নি।

**কিভাবে এরকম সমস্যা সমাধান করা যেতে পারে?**

Unsigned Integer ব্যবহার করা, যা সর্বাধিক মানকে 4,294,967,295 পর্যন্ত বাড়ায়। তবে আরও একটি scalable সমাধান হলো BIGINT ব্যবহার করা, যা অনেক বড় রেঞ্জ দিয়ে থাকে।
...
ডাটাবেসে সব ডাটা (টেবিলের রো, ইনডেক্সের এন্ট্রি ইত্যাদি) ফিক্সড সাইজের এক বা একাধিক ব্লকে স্টোর করা হয়। সেই ব্লকগুলো Page কে বলে। সাধারণত প্রতিটি page এর সাইজ 16KB হয়ে থাকে।

**Page দুই জায়গাতেই থাকে:**

- ডিস্কে (permanent/persistent storage): সব Pages-এর মূল কপি ডিস্কে স্টোর থাকে। এটাই ডাটাবেসের Durability নিশ্চিত করে অর্থাৎ যদি সার্ভার বন্ধ হয়ে যায় তখন ডাটা হারায় না।
- মেমোরিতে (in-memory cache/buffer pool): এখানে ডিস্ক থেকে আসা Page-গুলোর কপি থাকে।

**কিভাবে page-গুলো মেমোরিতে(buffer pool) আসে?**

- শুরুতে Buffer Pool খালি থাকে।
- প্রথম Query চলে → ৫টা Page ডিস্ক থেকে পড়ে Buffer Pool-এ আসে।
- পরের Query যদি সেই ৫টা Page-ই চায় → Buffer Pool নিজের কাছে থেকে দিয়ে দেয়।

**এখন Buffer Pool এর সাইজ এর তুলনায় যদি বেশি pages স্টোর হয়ে যায়?**

সেক্ষেত্রে নতুন Page লোড করার জন্য পুরোনো Page-কে Evict (বের করে দেওয়া) করতে হয়। Eviction Policy LRU (Least Recently Used) অ্যালগরিদম ব্যবহার করে হয়ে থাকে।
...
**একটি page-এর maximum সাইজ পর্যন্ত ডাটা ইন্সার্ট হয়ে গেলে কি হয়?**

আমরা জানি Cluster Index মানে ডাটার row গুলো ডিস্কে Primary Key order অনুযায়ী সাজানো অবস্থায় থাকে।

- Page A: id 1–100 (full size)
- Page B: id 101–200 (full size)

এখন id = 50 insert করলে, তখন Page Split হয়।
...
<p align="center">
  <img src="./images/mysql_error_log.png" alt="MySQL Error Log">
</p>
...
- Real-time কে slow করছে সেটা দেখা
- Log analyze করে root cause বের করা
...
slow_query_log তা ON এর মাধ্যমে log করা শুরু হবে। long_query_time 1, মানে হচ্ছে যেসব Query ১ second এর বেশি এর বেশি সময় নিবে তা log হবে।

সার্ভার restart এর প্রয়োজন নাই। প্রোডাকশন-এ কোয়েরি চলতেছে।

আমাদের এখন সেই ফাইলটা পড়তে হবে। ফাইল কোথায় পাবো?
...
যা বুজা যায়, SELECT \* FROM emails WHERE subject LIKE '%invoice%'; query যার Total execution time -> Query_time: 3.245678
...
<p align="center">
  <img src="./images/slow_query.png" alt="MySQL Slow Query">
</p>
