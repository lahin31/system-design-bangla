...
এটি একটি প্রসেস যেখানে এক বা একাধিক Producer থাকবে, যাদের কাজ হচ্ছে Message - Queue এর মধ্যে send করা এবং queue সেই রিকোয়েস্টগুলোকে প্রসেস করে বিভিন্ন consumer এর কাছে পাঠিয়ে দেয়।
...
এখন পর্যন্ত আমরা OAuth2-এর Flow দেখেছি।

কিন্তু একজন Senior Engineer শুধু **"কীভাবে কাজ করে"** জানেন না, বরং বোঝেন **"কেন এভাবে Design করা হয়েছে"** এবং **"Production-এ কী কী বিষয় মাথায় রাখতে হয়"**।

🔗 [**আরও পড়ুন: ওয়াথ২**](./sections/how-oauth2-works/README.md)
...
<p align="center">
  <img src="./images/best_db.png" alt="Best db">
</p>

## Section 42: Designing reliable Batch Processing for large-scale data

(নিজের experience থেকে) কয়েক বছর আগে একটা প্রজেক্টে কাজ করছিলাম, যেখানে একটা external API থেকে একসাথে ৫০,০০০ রেকর্ড fetch করতে হতো, তারপর সেগুলো process করে database-এ store করতে হতো।

শুরুতে মাথায় আসলো — আরে এটা আর কী, একবারে সব টেনে এনে কাজ শেষ করি। কিন্তু যত গভীরে গেলাম, তত বুঝলাম ব্যাপারটা এতটা সহজ না।

একসাথে ৫০ হাজার রেকর্ড memory-তে তুললে RAM-এ প্রচণ্ড চাপ পড়ে, Out of Memory error আসার সম্ভাবনা থাকে। তার উপর মাঝপথে যদি server crash করে বা network চলে যায়, তাহলে কিছু data process হবে আর কিছু হবে না — এই অর্ধেক-করা অবস্থাটা কোনো কাজের না, বরং আরও বিপজ্জনক। আর processing চলার সময় যদি মূল table-এর data কেউ বদলে ফেলে, তাহলে পুরো result হবে inconsistent — এটা তো একদমই মেনে নেওয়া যায় না।

তখন কয়েকটা জিনিস করলাম —

- প্রথমত, একবারে সব data না এনে ছোট ছোট batch-এ আনতে শুরু করলাম, ধরো ৫০০ বা ১০০০ করে। Memory-র উপর চাপ অনেকটাই কমে গেল।
- দ্বিতীয়ত, প্রতিটা batch সরাসরি process না করে Message Queue-তে পাঠিয়ে দিলাম। Worker সেখান থেকে একটা একটা করে তুলে কাজ করতো, শেষ হলে সব result একসাথে একটা row হিসেবে database-এ ঢুকতো। এতে কাজটা অনেকটা decoupled হলো, কোথাও failure হলেও queue থেকে আবার তোলা যেতো।
- তৃতীয়ত, প্রতিটা batch-এর processing পুরোটা একটা Transaction-এর মধ্যে রাখলাম। মাঝপথে কিছু একটা হলে পুরো transaction rollback হয়ে যাবে, partial data কোনোভাবেই database-এ ঢুকবে না।
- আর সবশেষে Transaction isolation level হিসেবে REPEATABLE READ বা SERIALIZABLE use করলাম, যাতে পুরো processing সময় জুড়ে data-র একটা consistent snapshot পাওয়া যায়। processing চলাকালীন অন্য কেউ data বদলে দিলেও আমার result-এ তার কোনো প্রভাব পড়বে না।

বড় কাজকে ছোট করে, queue দিয়ে ভাগ করে, transaction দিয়ে নিরাপদ রাখলে — এই তিনটা জিনিস ঠিকমতো করতে পারলে এই ধরনের বেশিরভাগ ঝামেলা এড়ানো সম্ভব।

<p align="center">
  <img src="./images/section-42.png" alt="Section 42">
</p>

## Section 43: High Availability best practices by Netflix
